...
namespace SimpleWeb
{
    ...
    public:
        ...
        public:
            ...
            inline bool IsRequestMetadata() { return bRequestMetadata; }
            inline void SetRequestMetadata(bool bReq) { bRequestMetadata = bReq; }

        private:
            template <typename... Args>
            Connection(std::shared_ptr<ScopeRunner> handler_runner_, long timeout_idle, Args &&...args) noexcept
                : handler_runner(std::move(handler_runner_)), socket(new socket_type(std::forward<Args>(args)...)), timeout_idle(timeout_idle), closed(false), bRequestMetadata(false) {}
