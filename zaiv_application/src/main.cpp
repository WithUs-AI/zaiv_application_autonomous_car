...
void draw_all_send_data(cv::Mat *frame, HailoROIPtr roi, std::list<Classes*> &classes)
{
	...
	std::vector<cv::Point> sorted_centers_1;

	// Find the point with highest y-coordinate and add to sorted list
	auto highest = std::max_element(centerPoint1.begin(), centerPoint1.end(), [](const cv::Point &a, const cv::Point &b) {
		return a.y < b.y;
	});

	if (highest != centerPoint1.end()) {
		sorted_centers_1.push_back(*highest);
		centerPoint1.erase(highest);
	}

	// While there are still points left, find the point closest to the last added point.
	while (!centerPoint1.empty()) {
		auto closest = std::min_element(centerPoint1.begin(), centerPoint1.end(), [&sorted_centers_1](const cv::Point &a, const cv::Point &b) {
			return cv::norm(a - sorted_centers_1.back()) < cv::norm(b - sorted_centers_1.back());
		});

		if (closest != centerPoint1.end()) {
			sorted_centers_1.push_back(*closest);
			centerPoint1.erase(closest);
		}
	}
	cv::polylines(*frame, sorted_centers_1, false, cv::Scalar(0, 255, 0), 2, 8, 0);
