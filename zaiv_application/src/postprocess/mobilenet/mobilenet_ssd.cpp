...
public:
    MobilenetSSDPost(HailoTensorPtr tensor, std::map<uint8_t, std::string> &labels_dict, float detection_thr = DEFAULT_MOBILENET_THRESHOLD, unsigned int max_boxes = DEFAULT_MAX_BOXES)
...
{
    auto post = MobilenetSSDPost(roi->get_tensor("ssd_Mobilenet/nms1"), common::coco_ninety_classes);
